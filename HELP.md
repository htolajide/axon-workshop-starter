...
*  [Axon Server](https://developer.axoniq.io/download), available to download as a standalone JAR file.
   ...
   In the Axon Server directory, create a file called `axonserver.properties` and the following line:
...
#### Commands and Events
The remaining unit tests cover the other commands and and events related to `Bike`.

Each test (uncomment to run) describes the expected commands parameters and events results. Use the Kotlin data classes `BikeRegisterCommand` and `BikeRegisteredEvent` as templates for implementing the other commands and events in the `rental` context.
...
A command handler is commonly paired with an event sourcing handler, where actual state updates are made; these handlers are also invoked by Axon upon _sourcing events_ from the event store when loading an aggregate.
...
> View the [JavaDoc](https://apidocs.axoniq.io/latest/org/axonframework/queryhandling/QueryGateway.html#query-java.lang.String-Q-org.axonframework.messaging.responsetypes.ResponseType-) for this method.

#### Projections
Queries are handled by a query model (a.k.a. _projections_). A projection is updated when events are published, and it implements so-called _query handlers_ that receive and respond to queries (from the `QueryGateway`).
...
*  Event handlers for each of the events currently supported; an event handler is annotated with `@EventHandler` and updates bike statuses in an embedded database.
   ...
   > `BikeStatus` contains methods to set relevant fields; you can an update a particular bike status via 
   >
...
Start the Bike application; you will see the application connected to Axon Server in _Overview_. 
...
*  Adding the corresponding command and event handlers in the `Payment` aggregate.
...
> You may be wondering why there isn't a `/{paymentId}/prepare` endpoint; these will covered in the _Process_ section below.
...
Update existing tests to reflect the new sequence of events; ensure these all tests pass (by fixing the `Bike` aggregate) before continuing.

> _Optionally_ add any tests you deem necessary!

The payment process is implemented in `PaymentProcess`; it's starts and ends upon receiving a `BikeRequestedEvent` and `PaymentConfirmedEvent` respectively.
